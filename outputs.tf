...
output "fluentbit_gke_exclusion_id" {
  ...
  value       = lookup(var.enable_exclusions, "fluentbit_gke", true) ? google_logging_project_exclusion.fluentbit_gke[0].id : null
}
output "fpm_exclusion_id" {
  description = "The ID of the FPM exclusion resource"
  value       = lookup(var.enable_exclusions, "fpm", true) ? google_logging_project_exclusion.fpm_exclusion[0].id : null
}
